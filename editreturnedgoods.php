...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    $SupplierBranch =$row["SupplierBranch"]; // Not available in returnedgoods, set to empty
    ...
    if (empty($ProductID) || empty($Product) || empty($PricePerProduct) || empty($SupplierName) || empty($SupplierBranch) || empty($Quantity) || empty($DeliveredAt)) {
        ...
        $sql_returnedgoods = "UPDATE returnedgoods " .
        ...
        $result_returnedgoods = $connection->query($sql_returnedgoods);

        if ($result_returnedgoods || $result_supplierbranch) {
